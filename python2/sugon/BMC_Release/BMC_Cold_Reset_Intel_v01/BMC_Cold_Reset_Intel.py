...
def get_args():
    parser = argparse.ArgumentParser(prog=None, usage="", description="The script includes Retry ME function.", epilog="eg: python %s -B 10.2.35.198 -H 10.2.33.145 -L '1,8' -M 3 -I" %(sys.argv[0]))
    parser.add_argument("-V","--version",dest="version",action="version",version="BMCCR_Intel_V1.0",help="show program's version number and exit")
    ...
    parser.add_argument("-M","--meretry",metavar="count",dest="meretry",type=int,action="store",default=3,help="ME retry function, set ME retry count")
    """ In-band test """
...
def collect_data(base=False, flag="lan", loop_num=None):
    ...
    for name,cmd in cmds.iteritems():
        if name == "lan info":
            ...
            if ME_Retry:    # ME_Retry>0
                ret = retry_test(cmd, retry_counts=ME_Retry, interval=5, flag=flag)
            else:           # ME_Retry=0
                ret = retry_test(cmd, retry_counts=1, flag=flag)
            if ret[0] != 0:
                show_warn_message("[%s] Run FAIL !" % cmd)
                data[name] = str(ret[-1])
                continue
        else:
            if flag == "lan":
    ...
    for name,res in data1.iteritems():
        ...
        if data2.has_key(name):
            ...
            if res == res2:
                ...
                msg = "ME info is not matched with baseline !"
...
def curl_check(base=False, loop_num=None):
    ...
    if ret[0] == 0:
        ...
        save_data(main_log, msg)
        return data
    check_list = ["GUID", "prodectname", "prodecttype", "MEVersion", "BMCVersion", "CPLDVersion", "BIOSVersion",
...
if __name__ == '__main__':
    Test_name = "BMC_Cold_Reset_Intel_v01"
    ...
    Test_time, ME_Retry, Lan_num, In_Band = \
        [args[item] for item in ["bmcip", "bmcuser", "bmcpasswd", "hostip", "osuser", "ospasswd", "time", "meretry", "lannum", "inband"]]
    """ In-band test """
    if In_Band:
        if not HOST_IP:
    ...
    ipmi_lan_cmds["me info"] = "%s -b 6 -t 0x2c mc info" %IPMITOOL
    ...
    ipmi_local_cmds["me info"] = "ipmitool -b 6 -t 0x2c mc info"
