...
function Usage(){
    ...
    echo -e "\teg: `basename $0` -B 10.2.35.198 -H 10.2.33.145 -R 3600 -I"
}
...
while getopts "L:B:R:U:P:H:Ih" name; do
    case ${name} in
        L)
            LAN_number=$OPTARG
        ;;
...
mylog="./BMC_Power-off_on-$BMC_IP.log"
...
echo "`date '+%D %r'`: Power off/on will first reboot" | tee -a $mylog
...
while [ `date +%s` -le "$endtime" ]; do
    ...
    echo -e "\033[1;31m start power off ...\033[0m"
#########################  begin Chassis Power CYCLE ##########################################
        
    ipmitool -I lanplus -H $BMC_IP -U "$USER" -P "$PASSWD" chassis power off
    ...
    if ! ${poweroff_flag}; then
        echo "[Cycle ${run}] Chassis power off FAIL (timeout:${downtime}), exit..." | tee -a $mylog
        exit 1
    fi
    ...
    echo -e "\033[1;32m start power on ...\033[0m"
    ipmitool -I lanplus -H $BMC_IP -U "$USER" -P "$PASSWD" chassis power on
    ...
    if [ "$result" != "0" ];then
        ...
        if [[ "$?" == 0 ]];then
            ...
            ipmitool -H $BMC_IP -U "$USER" -P "$PASSWD" chassis power on
            commandcycle=1
            readFanSpeed $run &
            fan_pid_again=$!
    ...
    if [[ "$commandcycle" == 1 ]];then
        ...
        retry
        if [ "$result" != "0" ];then
            echo -e "[\033[31m `date +"%D %T"` two power off/on still fail ,system may hang up \033[0m]" | tee -a $mylog
...
echo -e "[\033[32m `date +"%D %T"` CHASSIS Power off/on PASS\033[0m]" | tee -a $mylog
show_pass
mkdir LOG
{
mv cookie ./LOG/
